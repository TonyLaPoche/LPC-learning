...
type DebugZoneEditorArenaProps = {
  onExit: () => void;
};
...
export function DebugZoneEditorArena({ onExit }: DebugZoneEditorArenaProps) {
  ...
  const [labels, setLabels] = useState<ZoneLabelMap>(() =>
    getActiveZoneLabels(),
  );
  const [sel, setSel] = useState<Sel>({ id: "mouth", part: 0 });
  const [savedFlash, setSavedFlash] = useState(false);
  const [hasSaved, setHasSaved] = useState(() => hasSavedZoneCalibration());
  ...
  const zoneTitle = (id: PositionId, part: number, total: number) => {
    const base = labels[id] || DEFAULT_ZONE_LABELS[id];
  ...
  const resetFactory = () => {
    resetActiveZoneCalibration();
    setZones(cloneZoneRel(CALIBRATED_ZONE_REL));
    setLabels({ ...DEFAULT_ZONE_LABELS });
    setHasSaved(false);
    setSavedFlash(false);
  };

  const resetSelectedGeometry = () => {
    if (!zones) return;
    const factory = cloneZoneRel(CALIBRATED_ZONE_REL);
    setZones({
      ...zones,
      [sel.id]: factory[sel.id].map((r) => ({ ...r })),
    });
  };
  ...
  const saveToApp = () => {
    if (!zones) return;
    saveActiveZoneCalibration(zones, labels);
    setHasSaved(true);
    setSavedFlash(true);
    ...
    <div className="flex h-full min-h-0 flex-col gap-2">
      ...
      <div className="shrink-0 rounded-2xl border border-amber-400/40 bg-amber-400/10 p-3 sm:p-4">
        ...
        <h2 className="font-display text-xl font-bold leading-tight">
          Placer, renommer, sauvegarder
        </h2>
        <p className="mt-1 text-xs text-mist sm:text-sm">
          Comme l’outil 1, plus renommage des libellés.{" "}
          <strong className="text-foam">Save</strong> écrit en localStorage et
          s’applique tout de suite (parcours, détection, overlays).
        </p>
        {hasSaved && (
          <p className="mt-2 text-[11px] text-teal">
            Une calibration sauvegardée est active sur cet appareil.
          </p>
        )}
      </div>
          ...
          {absZones &&
            (Object.keys(absZones) as PositionId[]).flatMap((id) =>
              absZones[id].map((rect: NormRect, part: number) => {
                ...
                const label = zoneTitle(id, part, absZones[id].length);
                return (
                  <div
                    key={`${id}-${part}`}
      ...
      <div className="flex shrink-0 items-center gap-2 rounded-xl border border-panel-2/60 bg-panel/50 px-3 py-1.5">
        ...
        <label className="sr-only" htmlFor="face-zoom-debug-zone-editor">
          Zoom visage
        </label>
        <input
          id="face-zoom-debug-zone-editor"
          type="range"
          min={FACE_ZOOM_MIN}
          max={FACE_ZOOM_MAX}
          step={0.02}
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pb-4">
        <div className="flex flex-wrap gap-2">
          <button
            ...
            onClick={() =>
              setFrozen((f) =>
                f ? null : liveAnchors ? { ...liveAnchors } : null,
              )
            }
          ...
          >
            Reset géométrie zone
          </button>
          <button
            type="button"
            onClick={resetFactory}
            className="rounded-full border border-coral/40 px-3 py-1.5 text-xs text-coral hover:bg-coral/10"
          >
            Reset factory (tout)
          </button>
        </div>
        ...
        <div>
          ...
          <p className="mt-1 text-[10px] text-mist">
            Id technique : <code className="text-sky">{sel.id}</code> (inchangé
            pour le matching LPC)
          </p>
        </div>
        ...
        <div>
          <label
            htmlFor="zone-rename"
            className="mb-1.5 block text-[10px] font-semibold uppercase tracking-wider text-sky"
          >
            Renommer le libellé
          </label>
          <input
            ...
            value={labels[sel.id]}
            onChange={(e) =>
              setLabels((prev) => ({ ...prev, [sel.id]: e.target.value }))
            }
            className="w-full max-w-md rounded-xl border border-panel-2/80 bg-ink/60 px-3 py-2 text-sm text-foam outline-none focus:border-teal/50"
            placeholder={DEFAULT_ZONE_LABELS[sel.id]}
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            ...
            onClick={saveToApp}
            disabled={!zones}
            className="rounded-full bg-teal px-5 py-2.5 text-sm font-bold text-ink disabled:opacity-40"
          >
            Save — appliquer à l’app
          </button>
          {savedFlash && (
            <span className="text-sm font-semibold text-ok">
              Sauvegardé ✓ (actif partout)
            </span>
          )}
        </div>
